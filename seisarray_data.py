...
import signal_proc.window_functions as wf
...
class read_data():
	...
	def __init__(self,dirlist,fe,howmuch,ftype=None):
		...
		self.filext=fe
		self.mnames=dirlist
		self.arraydata=list(range(len(dirlist)))
	...
	def do_single_dir(self,filesdir,hm):
		...
		combined = list(zip(epdist_unsorted,file_list,self.stnames))
		...
		for i,x in enumerate(records):
			...
			try:
				...
				if abs(self.fulldata.shape[1]-len(tr.data))<0.01*samples:
					...
					trst=tr.stats.sac.b
					tret=tr.stats.sac.e
					# print(, tr.stats.station, trst, tret, tret-trst)
...
class plot_data():
	...
	def __init__(self,master,sdist,snames,deltat,tstart,figtitle,pn):
		...
		egdatamat=master[0]
		self.dataset_list=list(range(len(master)))
		self.pn=pn
		...
		for ed,d in enumerate(self.dataset_list):
			...
			for i in range(0,master[d].shape[0]):
				...
				single_trace = list(zip(self.time,seismgm))  # for sideways plotting
...
def see_subset(desd,rdo):
	...
	fd=desd[0]
	nspp=int(input("Number of seismograms per plot: "))
	nstot=fd.shape[0]
	nplots=int(nstot/nspp)
	if not nstot%nspp==0:
		nplots+=1
...
if __name__ == '__main__':
	...
	try:
		...
		fext=input("Extension for sac files in directory (Enter eg: .sac, .SAC, .LHZ ): ")
	desired_output = 0
	while desired_output != 3:
		...
		if desired_output>=3:
			...
			usrc_ww=int(input("See entire data (1) or a windowed portion (2) ? "))
			if usrc_ww==1:
			...
			usrc_hm=input("See only few stations at a time ? (y/n): ")
			if usrc_hm=='y':
			...
			usrcn=int(input("Enter 1 for Earth microseism noise or 2 for random Gaussian noise: " ))
